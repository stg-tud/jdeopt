 ...
 *
 * <p>
 * <tt>Class</tt> objects for array classes are not created by class loaders,
 * but are created automatically as required by the Java runtime. The class
 * loader for an array class, as returned by {@link
 * Class#getClassLoader()} is the same as the class loader for its element type;
 * if the element type is a primitive type, then the array class has no class
 * loader.
 *
 * <p>
 * Applications implement subclasses of <tt>ClassLoader</tt> in order to extend
 * the manner in which the Java virtual machine dynamically loads classes.
 *
 * <p>
 ...
 *
 * <p>
 * The <tt>ClassLoader</tt> class uses a delegation model to search for classes
 * and resources. Each instance of <tt>ClassLoader</tt> has an associated parent
 * class loader. When requested to find a class or resource, a
 * <tt>ClassLoader</tt> instance will delegate the search for the class or
 ...
 * parent of a <tt>ClassLoader</tt> instance.
 *
 * <p>
 * Class loaders that support concurrent loading of classes are known as
 * <em>parallel capable</em> class loaders and are required to register
 * themselves at their class initialization time by invoking the  {@link
 ...
 * class loaders need to be parallel capable, otherwise class loading can lead
 * to deadlocks because the loader lock is held for the duration of the class
 * loading process (see {@link #loadClass
 ...
 * should use the method {@link #defineClass <tt>defineClass</tt>} to create a
...
public abstract class ClassLoader {
     ...
     * <tt>checkCreateClassLoader</tt> method doesn't allow creation of a new
     * class loader.
     *
     * @since 1.2
     */
     ...
     * method {@link #getSystemClassLoader()
     ...
     * method searches for classes in the same manner as the {@link
     ...
     * built-in to the virtual machine is used, instead.  </p></li>
     *
     * <li><p>
     ...
     * {@link #getClassLoadingLock <tt>getClassLoadingLock</tt>} method during
     * the entire class loading process.
     *
     * @param name The <a href="#name">binary name</a> of the class
     *
     ...
     *
     * @throws NullPointerException If registered as parallel capable and
     * <tt>className</tt> is null
     *
     ...
     * the delegation model for loading classes, and will be invoked by the {@link #loadClass
     * <tt>loadClass</tt>} method after checking the parent class loader for the
     * requested class. The default implementation throws a
     ...
     * deprecated in favor of the version that takes a <a
     * href="#name">binary name</a> as its first argument, and is more secure.
     *
     * @param b The bytes that make up the class data. The bytes in positions
     * <tt>off</tt> through <tt>off+len-1</tt> should have the format of a valid
     ...
     *
     * @throws IndexOutOfBoundsException If either <tt>off</tt> or <tt>len</tt>
     * is negative, or if
     ...
     * certificates than this class, or if an attempt is made to define a class
     * in a package with a fully-qualified name that starts with
     * "{@code java.}".
     *
     * @see #loadClass(String, boolean)
     * @see #resolveClass(Class)
     ...
     *
     * <p>
     * To assign a specific <tt>ProtectionDomain</tt> to the class, use the {@link #defineClass(String, byte[], int, int,
     ...
     * an optional <tt>ProtectionDomain</tt>. If the domain is
     ...
     * must contain the same set of certificates or a
     * <tt>SecurityException</tt> will be thrown. Note that if
     * <tt>name</tt> is <tt>null</tt>, this check is not performed. You should
     * always pass in the <a href="#name">binary name</a> of the class you are
     * defining as well as the bytes. This ensures that the class you are
     * defining is indeed the class you think it is.
     *
     * <p>
     ...
     * the byte array "<tt>b</tt>", otherwise a {@link
     ...
     * @throws ClassFormatError If the data did not contain a valid class
     *
     * @throws NoClassDefFoundError If <tt>name</tt> is not equal to the
     * <a href="#name">binary name</a> of the class specified by <tt>b</tt>
     *
     * @throws IndexOutOfBoundsException If either <tt>off</tt> or <tt>len</tt>
     * is negative, or if
     * <tt>off+len</tt> is greater than <tt>b.length</tt>.
     ...
     * <tt>ProtectionDomain</tt>. If the domain is
     * <tt>null</tt>, then a default domain will be assigned to the class as
     * specified in the documentation for {@link #defineClass(String, byte[],
     * int, int)}. Before the class can be used it must be resolved.
     *
     ...
     * identical to those specified in the documentation for {@link
     ...
     * </tt></p>
     *
     * @param name The expected <a href="#name">binary name</a>. of the class,
     * or
     * <tt>null</tt> if not known
     *
     ...
     * linked as described in the "Execution" chapter of
     * <cite>The Java&trade; Language Specification</cite>.
     *
     ...
     * {@link #findResource(String)} to find the resource.  </p>
     *
     ...
     * <tt>null</tt> if the resource could not be found or the invoker doesn't
     ...
     * method doesn't allow access to this class loader's parent class loader.
     *
     * @since 1.2
     */
     ...
     * RuntimePermission#RuntimePermission(String)
     * <tt>RuntimePermission("getClassLoader")</tt>} permission to verify access
     * to the system class loader. If not, a
     * <tt>SecurityException</tt> will be thrown.  </p>
     *
     ...
     *
     * @throws IllegalStateException If invoked recursively during the
     * construction of the class loader specified by the
     * "<tt>java.system.class.loader</tt>" property.
     *
     * @throws Error If the system property "<tt>java.system.class.loader</tt>"
     * is defined but the named class could not be loaded, the provider class
     * does not define the required constructor, or an exception is thrown by
     * that constructor when it is invoked. The underlying cause of the error
     * can be retrieved via the {@link Throwable#getCause()} method.
     *
     * @revised 1.4
     */
    ...
    static void x_checkClassLoaderPermission() {
        SecurityManager sm = System.getSecurityManager();
        if (sm != null) {
            sm.checkPermission(SecurityConstants.GET_CLASSLOADER_PERMISSION);
        }
     ...
     * the system are entered into the <tt>systemNativeLibraries</tt>
     * vector.
     *
     * <p>
     ...
     * {@link #setPackageAssertionStatus(String, boolean)} or {@link
     * #setClassAssertionStatus(String, boolean)}.
     *
     * @param enabled
     * <tt>true</tt> if classes loaded by this class loader will henceforth have
     * assertions enabled by default, <tt>false</tt>
     ...
     * assertion status is to be set. A <tt>null</tt> value indicates the
     * unnamed package that is "current" (see section 7.4.2 of
     * <cite>The Java&trade; Language Specification</cite>.)
     *
     * @param enabled
     * <tt>true</tt> if classes loaded by this classloader and belonging to the
     * named package or any of its subpackages will have assertions enabled by
     * default, <tt>false</tt> if they will have assertions disabled by default.
     *
     * @since 1.4
     */
     ...
     * effect on the actual assertion status of any class. </p>
     *
     * @param className The fully qualified class name of the top-level class
     * whose assertion status is to be set.
     *
     * @param enabled
     * <tt>true</tt> if the named class is to have assertions enabled when (and
     * if) it is initialized, <tt>false</tt> if the class is to have assertions
     * disabled.
     *
     * @since 1.4
     */
     ...
     * status settings associated with the class loader. This method is provided
     * so that class loaders can be made to ignore any command line or
