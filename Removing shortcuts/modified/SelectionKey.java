 ...
 * selector. Cancelling a key does not immediately remove it from its selector;
 * it is instead added to the selector's <a
 * href="Selector.html#ks"><i>cancelled-key set</i></a> for removal during the
 * next selection operation. The validity of a key may be tested by invoking its
 ...
 * for readiness the next time one of the selector's selection methods is
 * invoked. The interest set is initialized with the value given when the key is
 * created; it may later be changed via the {@link
 *   #interestOps(int)} method. </p></li>
 *
 * <li><p>
 * The <i>ready set</i> identifies the operation categories for which the key's
 * channel has been detected to be ready by the key's selector. The ready set is
 ...
 * subclass of {@link SelectableChannel} defines an {@link
 ...
 * implementation, reading or writing the interest set may block indefinitely if
 * a selection operation is already in progress; in a high-performance
...
public abstract class SelectionKey {
     ...
     * channel is closed, or its selector is closed.  </p>
     *
     ...
     * effect. Once cancelled, a key remains forever invalid. </p>
     *
     * <p>
     ...
     * how long, is implementation-dependent.  </p>
     *
     ...
     *
     * @throws IllegalArgumentException If a bit in the set does not correspond
     * to an operation that is supported by this key's channel, that is, if
     ...
     * <p>
     * It is guaranteed that the returned set will only contain operation bits
     * that are valid for this key's channel.  </p>
     *
     ...
     * end-of-stream, has been remotely shut down for further reading, or has an
     * error pending, then it will add <tt>OP_READ</tt> to the key's
     ...
     *
     * <p>
     * An invocation of this method of the form <tt>k.isReadable()</tt>
     ...
     *
     * <p>
     * An invocation of this method of the form <tt>k.isWritable()</tt>
     * behaves in exactly the same way as the expression
     *
     * <blockquote><pre>{@code
    ...
    private static final AtomicReferenceFieldUpdater<SelectionKey, Object> attachmentUpdater = x_newUpdater(
    ...
    private static java.util.concurrent.atomic.AtomicReferenceFieldUpdater x_newUpdater(Class tclass, Class vclass, String fieldName) {
        try {
            return AccessController.doPrivileged(new PrivilegedExceptionAction<AtomicReferenceFieldUpdater>() {

                @Override
                public AtomicReferenceFieldUpdater run() throws PrivilegedActionException {
                    return x_getUpdater(tclass, vclass, fieldName);
                }
    ...
    private static java.util.concurrent.atomic.AtomicReferenceFieldUpdater x_getUpdater(Class tclass, Class vclass, String fieldName) {
