 ...
 * object, which implements the interface {@link InvocationHandler}. A method
 * invocation on a proxy instance through one of its proxy interfaces will be
 * dispatched to the {@link InvocationHandler#invoke
 * invoke} method of the instance's invocation handler, passing the proxy
 * instance, a {@code java.lang.reflect.Method} object identifying the method
 * that was invoked, and an array of type {@code Object} containing the
 * arguments. The invocation handler processes the encoded method invocation as
 * appropriate and the result that it returns will be returned as the result of
 ...
 *
 * <ul>
 * <li>Proxy classes are <em>public, final, and not abstract</em> if all proxy
 * interfaces are public.</li>
 *
 * <li>Proxy classes are <em>non-public, final, and not abstract</em> if any of
 * the proxy interfaces is non-public.</li>
 *
 ...
 * </pre> and the following cast operation will succeed (rather than throwing a
 ...
 * was passed to its constructor. The static
 * {@link Proxy#getInvocationHandler Proxy.getInvocationHandler} method will
 * return the invocation handler associated with the proxy instance passed as
 * its argument.
 *
 * <li>An interface method invocation on a proxy instance will be encoded and
 * dispatched to the invocation handler's {@link
 * InvocationHandler#invoke invoke} method as described in the documentation for
 * that method.
 *
 ...
 * becomes significant. When such a <i>duplicate method</i>
 * is invoked on a proxy instance, the {@code Method} object passed to the
 * invocation handler will not necessarily be the one whose declaring class is
 * assignable from the reference type of the interface that the proxy's method
 * was invoked through. This limitation exists because the corresponding method
 * implementation in the generated proxy class cannot determine which interface
 * it was invoked through. Therefore, when a duplicate method is invoked on a
 * proxy instance, the {@code Method} object for the method in the foremost
 * interface that contains the method (either directly or inherited through a
 * superinterface) in the proxy class's list of interfaces is passed to the
 * invocation handler's {@code invoke} method, regardless of the reference type
...
public class Proxy implements java.io.Serializable {
     ...
     * interfaces. If any of the given interfaces is non-public, the proxy class
     * will be non-public. If a proxy class for the same permutation of
     * interfaces has already been defined by the class loader, then the
     * existing proxy class will be returned; otherwise, a proxy class for those
     * interfaces will be generated dynamically and defined by the class loader.
     *
     * <p>
     ...
     * <ul>
     * <li> the given {@code loader} is {@code null} and the caller's class
     * loader is not {@code null} and the invocation of {@link SecurityManager#checkPermission
     *             s.checkPermission} with {@code RuntimePermission("getClassLoader")}
     * permission denies access.</li>
     * <li> for each proxy interface, {@code intf}, the caller's class loader is
     * not the same as or an ancestor of the class loader for {@code intf} and
     * invocation of {@link SecurityManager#checkPackageAccess
     *             s.checkPackageAccess()} denies access to {@code intf}.</li>
     * </ul>
     *
     * @throws NullPointerException if the {@code interfaces} array argument or
     * any of its elements are {@code null}
     */
    public static Class<?> getProxyClass(ClassLoader loader,
        ...
        if (sm != null) {
            x_checkProxyAccess(loader, interfaces);
        }
    ...
    private static void x_checkProxyAccess(ClassLoader loader, Class<?>... interfaces) {
        SecurityManager sm = System.getSecurityManager();
        if (sm != null) {
        ...
        ReflectUtil.x_checkProxyPackageAccess(interfaces);
    }
     ...
     * class that is defined by the specified class loader and that implements
     ...
     * invocation of {@link SecurityManager#checkPackageAccess
     *               s.checkPackageAccess()} denies access to {@code intf};</li>
     * <li> any of the given proxy interfaces is non-public and the caller class
     * is not in the same {@linkplain Package runtime package} as the non-public
     * interface and the invocation of
     * {@link SecurityManager#checkPermission s.checkPermission} with
     * {@code ReflectPermission("newProxyInPackage.{package name}")} permission
     * denies access.</li>
     * </ul>
     * @throws NullPointerException if the {@code interfaces} array argument or
     * any of its elements are {@code null}, or if the invocation handler,
     * {@code h}, is {@code null}
     */
    ...
    private static void x_checkNewProxyPermission(Class<?> proxyClass) {
        SecurityManager sm = System.getSecurityManager();
        if (sm != null) {
            if (ReflectUtil.isNonPublicProxyClass(proxyClass)) {
                int n = proxyClass.getName().lastIndexOf('.');
     ...
     * class loader for the invocation handler and invocation of {@link SecurityManager#checkPackageAccess
