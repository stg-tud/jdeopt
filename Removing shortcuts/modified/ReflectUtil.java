...
public final class ReflectUtil {
    ...
    /**
     * Does a conservative approximation of member access check. Use this if you
     * don't have an actual 'userland' caller Class/ClassLoader available. This
     * might be more restrictive than a precise member access check where you
     * have a caller, but should never allow a member access that is forbidden.
     *
    ...
    /**
     * Returns true if package access check is needed for reflective access from
     * a class loader 'from' to classes or members in a class defined by class
     * loader 'to'. This method returns true if 'from' is not the same as or an
     * ancestor of 'to'. All code in a system domain are granted with all
     * permission and so this method returns false if 'from' class loader is a
     * class loader loading system classes. On the other hand, if a class loader
     * attempts to access system domain classes, it requires package access
     * check and this method will return true.
     */
     ...
     * {@code SecurityException} if it accesses a restricted package from the
    ...
    public static void x_checkProxyPackageAccess(Class<?>... interfaces) {
        SecurityManager sm = System.getSecurityManager();
        if (sm != null) {
            for (Class<?> intf : interfaces) {
                checkPackageAccess(intf);
    ...
    /**
     * Checks if {@code Class cls} is a VM-anonymous class as defined by
     * {@link sun.misc.Unsafe#defineAnonymousClass} (not to be confused with a
     * Java Language anonymous inner class).
     */
